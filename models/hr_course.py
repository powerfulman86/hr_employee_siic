...
class HrCourse(models.Model):
    ...
    category_id = fields.Many2one("hr.course.category", string="Category", required=True, )

    start_date = fields.Date(string="Start date", readonly=True, states={"draft": [("readonly", False)]},
                             tracking=True, )
    end_date = fields.Date(string="End date", readonly=True, states={"draft": [("readonly", False)]}, tracking=True, )
    currency_id = fields.Many2one("res.currency", string="Currency",
                                  default=lambda self: self.env.user.company_id.currency_id, )
    cost = fields.Monetary(string="Course Cost", required=True, tracking=True)
    authorized_by = fields.Many2one(string="Authorized by", comodel_name="hr.employee", required=True, readonly=True,
                                    states={"draft": [("readonly", False)]}, tracking=True, )
    permanence = fields.Boolean(string="Has Permanence", readonly=True, states={"draft": [("readonly", False)]},
                                tracking=True, help="Check if the participants of this course are restricted to"
                                                    " stay in the company for a certain period of time.", )
    permanence_time = fields.Char(string="Permanence time", readonly=True, states={"draft": [("readonly", False)]},
                                  tracking=True,
    ...
    attendant_ids = fields.Many2many("hr.employee", readonly=True,
                                     states={"waiting_attendees": [("readonly", False)]}, )
    course_attendee_ids = fields.One2many("hr.course.attendee", inverse_name="course_id", readonly=True,
    ...
    def _waiting2inprogress_values(self):
        ...
        if deleted_attendees != "":
            message = (_("Employees removed from this course: <br></br>%s") % deleted_attendees)
    ...
    def create(self, values):
        res = super(HrCourse, self).create(values)
        res.name = self.env['ir.sequence'].next_by_code('hr.course') or '/'
        return res
...
class HRCourseAttendee(models.Model):
    ...
    course_id = fields.Many2one("hr.course", ondelete="cascade", readonly=True, required=True, )
